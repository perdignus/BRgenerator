...
public class AttributeRangeDAO extends RootDAO {
	...
	public BusinessRule getData(int id) {
		...
		try (Connection con = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD)) {
			 ...
			 br = new AttributeRangeRule(dbResultSet.getString("RuleName"),
			 dbResultSet.getInt("BusinessRuleNr"), dbResultSet.getString("TableName"),
			 dbResultSet.getString("ColumnName"), dbResultSet.getInt("MinimumValue"),
			 dbResultSet.getInt("MaximumValue"),
			 (dbResultSet.getString("CompareType"))=="");
			 break;
			 }

			// ^database is nog niet compleet
			if (id == 0) {
				br = new AttributeRangeRule("name", id, "RootTable", "RootColumn", 0, 2, true);
			} else
				br = new AttributeRangeRule("TEST", id, "RootTEST", "RootTEST", 4, 2, false);
